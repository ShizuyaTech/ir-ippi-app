                            ...
                            <form action="{{ route_encrypted('feedback.update', $feedback->id) }}" method="POST">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="category" class="form-control selectric">
                                                ...
                                                @foreach (App\Models\Feedback::CATEGORIES as $key => $value)
                                                    <option value="{{ $key }}"
                                                        {{ old('category', $feedback->category) == $key ? 'selected' : '' }}>
                                                        {{ $value }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="priority" class="form-control selectric">
                                                ...
                                                @foreach (App\Models\Feedback::PRIORITIES as $key => $value)
                                                    <option value="{{ $key }}"
                                                        {{ old('priority', $feedback->priority) == $key ? 'selected' : '' }}>
                                                        {{ $value }}
                                                    </option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="content" rows="8" class="form-control summernote"
                                        placeholder="Tuliskan detail feedback/masukan untuk manajemen...">{{ old('content', $feedback->content) }}</textarea>
                                    @error('content')
                                        <div class="invalid-feedback" style="display: block;">
                                            {{ $message }}
                                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="status" class="form-control selectric">
                                        <option value="draft"
                                            {{ old('status', $feedback->status) == 'draft' ? 'selected' : '' }}>Simpan
                                            sebagai Draft</option>
                                        <option value="submitted"
                                            {{ old('status', $feedback->status) == 'submitted' ? 'selected' : '' }}>Kirim
                                            ke Manajemen</option>
                                    </select>
                                    @error('status')
                                        <div class="invalid-feedback" style="display: block;">
